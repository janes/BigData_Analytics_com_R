...
df <- data.frame(read.table(file = 'http://data.princeton.edu/wws509/datasets/effort.dat', header = TRUE, sep = ","))
df
...
plot(dfIris$Sepal.Length, dfIris$Sepal.Width)
...
?subset
# traz uma determinada coluna
dfSubset = subset(iris, iris$Sepal.Length > 7, select = c(Sepal.Length))
dfSubset
#Traz todas as colunas
dfSubset2 = subset(iris, iris$Sepal.Length > 7)
dfSubset2
...
RSiteSearch("filter")
dfFilter <- filter(dfNovo, dfNovo$Sepal.Length > 6)
dfFilter
dfFilter2 <- filter(dfNovo, dfNovo$Sepal.Length > 5)
dfFilter2