## Funcoes e utilitarios para analise de dados do dataset German credit data


# Funcao para converter variaveis numericas para fator
quantize.num <- function(x, nlevs = 5, maxval = 1000, minval = 0, ordered = TRUE) {
  cuts <- seq(min(x), max(x), length.out = nlevs + 1)
  cuts[1] <- minval
  cuts[nlevs + 1] <- maxval
  print(cuts)
  x <- cut(x, breaks = cuts, ordered_result = ordered)
}
...
# funcao anonima para testar o tipo de dado de cada coluna no dataset
...
equ.Frame <- function(in.frame, nrep) {
  ...
  if(nrep > 0) {
    ...
    #in.frame <- data.frame(Map(function(x, y) {
    #  rbind(x, rep(y, nrep)) },
    #  in.frame, posFrame))
  }
  in.frame
...
serList <- function(serlist) {
  ...
  if(!is.list(serlist) | is.null(serlist) | length(serlist) < 1) {
    warning(messages[2])
    return(data.frame(as.integer(serialize(list(numElements = 0,
                                                payloa = NA,
                                                connection = NULL)))))
  }
  
  # Encontando o numero de objetos
  nObj <- length(serlist)
  
  tryCatch(outframe <- data.frame(payload = as.integer(serialize(list(numElements = nObj,
                                                                      payload = serlist),
                                                                 connection = NULL))),
           error = function(e) {
             warning(messages[3])
             outframe <- data.frame(payload = as.integer(serialize(list(numElements = 0,
                                                                        payload = NA),
                                                                   connection = NULL)))
           }
  )
