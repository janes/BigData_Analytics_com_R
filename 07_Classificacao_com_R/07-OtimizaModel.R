# Otimizando o modelo preditivo
# ... cont do script 06


# Modelo randomForest ponderado
# o pacote C50 permite que vc de peso aos erros, construindo assim um resultado ponderado
install.packages("C50")
library(C50)

# Criando um Cost Function
Cost_func <- matrix(c(0, 1.5, 1, 0), nrow = 2, dimnames = list(c("1", "2"), c("1", "2")))

# Criando o Modelo 
? randomForest
? C5.0

# Modelo
modelo_v2 <- C5.0(CreditStatus ~ CheckingAcctStat
                  + Purpose
                  + CreditHistory
                  + SavingsBonds
                  + Employment,
                  data = dados_treino,
                  trials = 100,
                  cost = Cost_func)

print(modelo_v2)

# dataframes com valores atuais e previstos
library(randomForest)
library(C50)

result_previsto_v2 <- data.frame(actual = dados_test$CreditStatus,
                                 previsto = predict(object = modelo_v2,
                                                    newdata = dados_test))

# Calculando a confusion matrix (existem outras formas)

# Label 1 - Credito Ruim
# Label 2 - Credito Bom

# Formulas
Accuracy <- function(x) {
  (x[1,1] + x[2,2]) / (x[1,1] + x[1,2] + x[2,1] + x[2,2])
}

Recall <- function(x) {
  x[1,1] / (x[1,1] + x[1,2])
}

Precision <- function(x) {
  x[1,1] / (x[1,1] + x[2,1])
}

W_Accuracy <- function(x) {
  (x[1,1] + x[2,2]) / (x[1,1] + 5 * x[1,2] + x[2,1] + x[2,2])
}

F1 <- function(x) {
  2 * x[1,1] / (2 * x[1,1] + x[1,2] + x[2,1])
}

# Criando a confusion matrix
confMat_v2 <- matrix(unlist(Map(function(x, y) {
  sum(ifelse(result_previsto_v2[, 1] == x &
               result_previsto_v2[, 2] == y, 1, 0))},
  c(2, 1, 2, 1), c(2, 2, 1, 1))), nrow = 2)

# Criando um dataframe com as estatisticas dos testes
dt_mat_2 <- data.frame(Category = c("Credito Ruim", "Credito Bom"),
                     Classificado_como_ruim = c(confMat_v2[1,1], confMat_v2[2,1]),
                     Classificado_como_bom = c(confMat_v2[1,2], confMat_v2[2,2]),
                     Accuracy_Recall = c(Accuracy(confMat_v2), Recall(confMat_v2)),
                     Precision_WAcc = c(Precision(confMat_v2), W_Accuracy(confMat_v2)))


print(dt_mat_2)


# Gerando Confusion Matrix com caret
library(caret)
confusionMatrix(result_previsto_v2$actual, result_previsto_v2$previsto)













