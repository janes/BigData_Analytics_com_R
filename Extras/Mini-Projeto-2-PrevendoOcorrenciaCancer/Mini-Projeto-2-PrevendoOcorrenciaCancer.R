# Projeto 2 - Prevendo Ocorrencia de Cancer
...
# Criando dados de treino e dados de teste (os dados ja estão de forma aleatoria)
...
# A funcao knn() retorna um objeto do tipo fator com as previsoes para cada exemplo no dataset de teste
class(modelo)
...
# Usaremos amostra com 100 observacoes: length(dados_teste_labels)
...
dados_treino_z <- dados_z[1:469, ]
dados_teste_z <- dados_z[470:569, ]

dados_treino_z_labels <- dados[1:469, 1]
dados_teste_z_labels <- dados[470:569, 1]

# Reclassificando
modelo_v2 <- knn(train = dados_treino_z,
                 test = dados_teste_z,
                 cl = dados_treino_z_labels,
                 k = 21)

# Criando uma tabela cruzada dos dados previstos x dados atuais
CrossTable(x = dados_teste_z_labels, y = modelo_v2, prop.chisq = FALSE)
...
# A medida que aumentamos k, diminuimos a taxa de erro do modelo
