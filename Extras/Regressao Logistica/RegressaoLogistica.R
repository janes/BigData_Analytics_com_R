...
# Vamos prever uma classificacao - sobreviventes e nao sobreviventes
...
# Utilizaremos o pacote Amelia e suas funcoes para definir o volume de dados "Missing"
# Cerca de 20% dos dados sobre idade estao faltando (Missing)
...
p1 <- ggplot(dados_treino, aes(Pclass, Age)) + geom_boxplot(aes(group = Pclass, fill = factor(Pclass), alpha = 0.4))
...
# Usaremos esta medida para imputar as idades Missing
...
missmap(dados_treino, main = "Titanic Training Data - Mapa de dados Midding",
        col = c("yellow", "black"), legend = FALSE)
